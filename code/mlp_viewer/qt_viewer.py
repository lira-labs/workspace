...
class GraphNodeItem(QGraphicsEllipseItem):
    def __init__(self, node: Dict[str, Any]):
        ...
        self.label_item.setPos(
            -self.label_item.boundingRect().width() / 2, size / 2 + 4
        )
...
class GraphEdgeItem(QGraphicsLineItem):
    def __init__(self, src: Dict[str, Any], dst: Dict[str, Any], edge: Dict[str, Any]):
        ...
        poly = QPolygonF(
            [QPointF(tip_x, tip_y), QPointF(left_x, left_y), QPointF(right_x, right_y)]
        )
...
class GraphScene(QGraphicsScene):
    ...
    def build(self, network: Network):
        ...
        if node_items_raw:
            input_layer_nodes = [
                pair for pair in node_items_raw if pair[0]["layer"] == 0
            ]
            ...
            for idx, (raw, item) in enumerate(input_layer_nodes):
                ...
                left_label.setPos(
                    item.x() - item.rect().width() / 2 - br.width() - 12,
                    item.y() - br.height() / 2,
                )
                ...
                feat_label.setPos(
                    item.x() - fr.width() / 2,
                    item.y() - item.rect().height() / 2 - fr.height() - 6,
                )
                self.addItem(feat_label)
        # label de saída
        output_layer_index = (
            max(n["layer"] for n, _ in node_items_raw) if node_items_raw else 0
        )
        output_nodes = [
            pair for pair in node_items_raw if pair[0]["layer"] == output_layer_index
        ]
        if output_nodes:
            ...
            out_label.setPos(
                item.x() + item.rect().width() / 2 + 12, item.y() - orc.height() / 2
            )
    ...
    def highlight_neuron(self, layer_index: int, neuron_index: int, network: Network):
        ...
        self.highlight_connections_to(
            neuron=layer.neurons[neuron_index], network=network
        )
    ...
    def highlight_connections_to(self, neuron, network: Network):
        ...
        for c in network._by_target.get(neuron.id, []):
            for e in self.edge_items:
                if (
                    hasattr(e, "weight_label")
                    and c.source.id in e.toolTip()
                    and c.target.id in e.toolTip()
                ):
                    pen = e.pen()
...
class NetworkViewerWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.spin_add_layer_size = QSpinBox()
        self.spin_add_layer_size.setMinimum(1)
        self.spin_add_layer_size.setValue(2)
        self.spin_add_layer_pos = QSpinBox()
        self.spin_add_layer_pos.setMinimum(1)
        ...
        self.spin_remove_layer = QSpinBox()
        self.spin_remove_layer.setMinimum(1)
        ...
        self.spin_add_neuron_layer = QSpinBox()
        self.spin_add_neuron_layer.setMinimum(1)
        ...
        self.spin_remove_neuron_layer = QSpinBox()
        self.spin_remove_neuron_layer.setMinimum(1)
        self.spin_remove_neuron_index = QSpinBox()
        self.spin_remove_neuron_index.setMinimum(0)
        ...
        self.combo_pep_current_changed = self.combo_prep.currentTextChanged.connect(
            self._on_change_prep
        )
        ...
        if kind == "neuron":
            ...
            if hasattr(self, "_current_target") and self._current_target is not None:
                ...
                self.label_status.setText(
                    f"Concluído. Output={out_val:.4f} Erro={err:.4f}"
                )
            else:
    ...
    def _on_load_dataset(self):
        try:
            ...
            if self.network.dataset_inputs:
                ...
                self.label_status.setText(
                    f"Dataset carregado: {len(self.network.dataset_inputs)} samples"
                )
            else:
    ...
    def _parse_inputs_text(self) -> List[float]:
        ...
        if len(vals) != len(self.network.layers[0]):
            raise ValueError(
                f"esperado {len(self.network.layers[0])} valores, recebido {len(vals)}"
            )
        return vals
    ...
    def _on_apply_inputs(self):
        if self._forward_started:
            QMessageBox.information(
                self, "Info", "Reset antes de aplicar novos inputs."
            )
            return
        try:
    ...
    def _ensure_sample_for_training(self):
        ...
        if self._dataset_loaded:
            ...
            if not self.edit_inputs.text().strip():
                QMessageBox.information(
                    self, "Info", "Forneça inputs ou carregue dataset para treinar."
                )
                return None
            try:
                x = self._parse_inputs_text()
            ...
            QMessageBox.information(
                self,
                "Info",
                "Sem target (dataset). Não é possível calcular erro/backprop.",
            )
            return None
    ...
    def _on_train_step(self):
        if self._forward_started:
            QMessageBox.information(
                self, "Info", "Use Reset antes de treinar (forward em andamento)."
            )
            return
        ...
        self.label_status.setText(
            f"TrainStep out={outputs[0]:.4f} target={target:.4f} erro={error:.4f} loss={loss:.5f}"
        )
    ...
    def _on_train_epoch(self):
        ...
        if self.network.dataset_inputs:
            self.scene.update_input_values(
                self.network.transform_inputs(self.network.dataset_inputs[-1])
            )
    ...
    def _on_change_prep(self, name: str):
        if self._forward_started:
            ...
            idx = self.combo_prep.findText(
                getattr(self.network, "preprocess_mode", "none")
            )
    ...
    def _ensure_loss_window(self):
        if self._loss_window is None:
            ...
            self.loss_text = QPlainTextEdit()
            self.loss_text.setReadOnly(True)
