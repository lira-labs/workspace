...
class Network:
    def __init__(self, layer_sizes: List[int]):
        ...
        self.learning_rate: float = 0.01  # default LR
        self._wire()
    ...
    def _rebuild_connection_index(self):
        ...
        for c in self.connections:
            ...
            self._by_source.setdefault(c.source.id, []).append(c)
    ...
    def remove_neuron(self, layer_index: int, neuron_index: int):
        ...
        self.connections = [
            c
            for c in self.connections
            if c.source is not target and c.target is not target
        ]
    ...
    def summary(self) -> str:
        parts = []
        parts.append("layers:" + ", ".join(str(len(layer)) for layer in self.layers))
    ...
    def forward_step(self):
        ...
        if self._fp_neuron_index < len(layer):
            ...
            result = (
                "neuron",
                self._fp_layer_index,
                self._fp_neuron_index,
                neuron.output,
            )
    ...
    # ---- backpropagation ----
    def _activation_derivative(self, neuron: Neuron) -> float:
        if self.activation_name == "sigmoid":
            return neuron.output * (1.0 - neuron.output)
        elif self.activation_name == "relu":
            return 1.0 if neuron.input_sum > 0 else 0.0
        return 1.0

    def _compute_deltas(self, target: float) -> float:
        if len(self.layers[-1]) != 1:
            raise NotImplementedError("backward suporta apenas 1 neurônio de saída")
        output_neuron = self.layers[-1].neurons[0]
        o = output_neuron.output
        deriv_out = self._activation_derivative(output_neuron)
        error = o - target
        output_neuron.delta = error * deriv_out

        # ocultas - usa _by_source para evitar loop em todas conexões
        for li in range(len(self.layers) - 2, 0, -1):
            layer = self.layers[li]
            for neuron in layer.neurons:
                s = 0.0
                for c in self._by_source.get(neuron.id, []):
                    if c.target in self.layers[li + 1].neurons:
                        s += c.weight * c.target.delta
                neuron.delta = s * self._activation_derivative(neuron)
        return error
    ...
    def train_step(
        self, inputs: List[float], target: float, learning_rate: float = 0.01
    ):
        ...
        for c in self.connections:
            grads[c.id] = c.source.output * c.target.delta
    ...
    # ---- ativação ----
    def _activation_derivative_from_output(
        self, output: float, input_sum: float
    ) -> float:
        if self.activation_name == "sigmoid":
    ...
    def to_dict(self) -> Dict:
        sizes = [len(layer) for layer in self.layers]
    ...
    def load_json(self, path: str):
        ...
        if sizes and sizes != [len(layer) for layer in self.layers]:
